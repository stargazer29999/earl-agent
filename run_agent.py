...
class AIAgent:
    ...
    def run_conversation(
        ...
        # Earl Agent: Route task before execution
        try:
            from earl_router import TaskRouter
            if hasattr(self, "config") and self.config:
                router = TaskRouter(self.config)
                # Estimate context length roughly
                ctx_len = len(str(conversation_history)) if conversation_history else 0
                tier = router.classify(user_message, context_length=ctx_len)
                router.inject_router_config(self, tier)
                import logging
                logging.getLogger("earl_router").info(f"Routed task to tier: {tier.value} ({self.provider}/{self.model})")
        except Exception as e:
            pass
...
def main(
    ...
    # Run conversation
    if orchestrate:
        print("\n👑 Starting Orchestrator mode (Sun Tzu)...")
        from earl_orchestrator import Orchestrator
        import uuid
        def factory():
            return AIAgent(
                base_url=base_url,
