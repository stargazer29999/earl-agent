...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    _soul_loaded = False
    if hasattr(agent, "_profile_system_prompt") and agent._profile_system_prompt:
        stable_parts.append(agent._profile_system_prompt)
        _soul_loaded = True
    ...
    # Earl Agent: Inject active plan context into the volatile prompt
    try:
        if hasattr(agent, "state_db") and agent.state_db and agent.session_id:
            from earl_planner import PlanManager
            planner = PlanManager(agent.state_db)
            plan_context = planner.get_plan_context(agent.session_id)
            if plan_context:
                volatile_parts.append(plan_context)
    except Exception as e:
        import logging
        logging.getLogger("earl_planner").debug(f"Failed to inject plan context: {e}")
